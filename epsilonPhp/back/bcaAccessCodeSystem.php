...
{
	$nextChallengeNumber = intval($accessCodeArrayed[$selectedCourse]) + 1;
...
function numberToRankNamed($numberFromArray)
{
	...
	if ($numberFromArray == 0) {
		$rankNamed = '<span class="disabled">' . $rank0 . '</span>';
	}
	if ($numberFromArray == 1) {
		$rankNamed = '<span class="enabled">' . $rank1 . '</span>';
	}
	if ($numberFromArray == 2) {
		$rankNamed = '<i class="fa fa-graduation-cap"></i> <span class="' . $rank2 . '">' . $rank2 . '</span>';
	}
	if ($numberFromArray == 3) {
		$rankNamed = '<i class="fa fa-handshake"></i> <span class="' . $rank3 . '">' . $rank3 . '</span>';
	}
	if ($numberFromArray == 4) {
		$rankNamed = '<i class="fa fa-hand-holding"></i> <span class="' . $rank4 . '">' . $rank4 . '</span>';
	}
	if ($numberFromArray == 5) {
		$rankNamed = '<i class="fa fa-star"></i> <span class="' . $rank5 . '">' . $rank5 . '</span>';
	}
	return $rankNamed;
}
...
function displayCoursesList($accessCodeArrayed)
{
	...
	echo '<ul id="parcours-list">
		<li><strong>' . $course0 . ' : </strong>' . numberToRankNamed($accessCodeArrayed[0]) . displayCourseLink(0, $accessCodeArrayed) . ' <span class="see"><i class="fas fa-eye"></i> <a href="listingUserFile.php">Voir</a></span></li>';
